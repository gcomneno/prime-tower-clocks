"""
JSONL signature I/O + validation for Prime Tower Clocks (PTC).

File format (v1):
  - First line: header {"type":"ptc","version":1,"base":2,...}
  - Next lines: clock records {"p":..., "z":true} or {"p":..., "z":false, "e":...}
  - Optional last line: summary {"type":"summary","k":...,"M_bits":...,"N_bits":...}
"""
...
def dump_signature_jsonl(sig: PTCSig, path: str, include_summary: bool = True) -> None:
    ...
    header = {"type": "ptc", "version": 1, "base": sig.base}
    if sig.created_utc:
    ...
    with open(path, "w", encoding="utf-8") as f:
        ...
        if include_summary:
            summary = {"type": "summary", "k": len(sig.clocks)}
...
def load_signature_jsonl(path: str) -> PTCSig:
    ...
    with open(path, encoding="utf-8") as f:
        for lineno, line in enumerate(f, start=1):
            ...
            # after header: either clock or summary
            if obj.get("type") == "summary":
                summary = _validate_summary(obj)
                # allow summary only as last meaningful record
                continue
    ...
    # merge summary into header struct
